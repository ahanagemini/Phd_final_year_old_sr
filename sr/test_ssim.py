...
def test_ssim(image_true, image_pred):
    criterion = SSIM()
    # case 1 same image
    y_pred = []
    for i in range(5):
    ...
    print("same image SSIM loss is {}".format(np.abs(loss)))
    ...
    print("different image SSIM loss is {}".format(np.abs(loss)))

    # case 3 actual data same image
