...
class EDSR(nn.Module):
    def __init__(
        ...
        self.msa = attention.PyramidAttention(channel=n_feats, reduction=8, res_scale=0.1, ksize=kernel_size)
        ...
        # define body module
        m_body = [
            common.ResBlock(conv, n_feats, kernel_size, act=act, res_scale=0.1)
        ...
        self.last_conv = conv(tail_feats, 1, kernel_size)
    ...
    def forward(self, x):
        ...
        for _ in range(self.upsample_debt):
            x = self.tail(res)
            res = x
        # x = self.add_mean(x)
        x = self.last_conv(x)
