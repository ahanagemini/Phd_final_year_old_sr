...
from . import common
from .tools import extract_image_patches, reduce_mean, reduce_sum, same_padding
...
class PyramidAttention(nn.Module):
    def __init__(
        ...
        self.conv_match_L_base = common.BasicBlock(
            conv, channel, channel // reduction, 1, bn=False, act=nn.PReLU()
        )
        self.conv_match = common.BasicBlock(
            conv, channel, channel // reduction, 1, bn=False, act=nn.PReLU()
        )
        self.conv_assembly = common.BasicBlock(
            conv, channel, channel, 1, bn=False, act=nn.PReLU()
        )
    ...
    def forward(self, input):
        ...
        for idx, xi in enumerate(input_groups):
            # group in a filter
            wi = torch.cat(
                [w[i][idx][0] for i in range(len(self.scale))], dim=0
            )  # [L, C, k, k]

            # normalize
            max_wi = torch.max(
                torch.sqrt(reduce_sum(torch.pow(wi, 2), axis=[1, 2, 3], keepdim=True)),
                self.escape_NaN,
            )
            wi_normed = wi / max_wi
            # matching
            ...
            yi = F.conv2d(
                xi, wi_normed, stride=1
            )  # [1, L, H, W] L = shape_ref[2]*shape_ref[3]

            yi = yi.view(
                    1, wi.shape[0], yi.shape[2], yi.shape[2]
            #    1, wi.shape[0], shape_base[2], shape_base[3]
            ...
            yi = F.conv_transpose2d(yi, raw_wi, stride=self.stride, padding=1) / 4.0
            y.append(yi)
