...
class DataGenerator(Dataset):
    ...
    def __init__(self, conf, gan):
        ...
        (
            ...
            self.crop_indices_for_d,
        ) = self.make_list_of_crop_indices(conf=conf)
    ...
    def next_crop(self, for_g, idx):
        ...
        crop_im = im2tensor(crop_im)
        crop_im = swap_axis(crop_im)
        return crop_im
    ...
    def make_list_of_crop_indices(self, conf):
        ...
        crop_indices_for_g = np.random.choice(
            a=len(prob_map_sml), size=iterations, p=prob_map_sml
        )
        crop_indices_for_d = np.random.choice(
            a=len(prob_map_big), size=iterations, p=prob_map_big
        )
    ...
    def create_prob_maps(self, scale_factor):
        ...
        prob_map_sml = create_probability_map(
            nn_interpolation(loss_map_sml, int(1 / scale_factor)), self.g_input_shape
        )
    ...
    def shave_edges(self, scale_factor, real_image):
        ...
        self.input_image = (
            self.input_image[: -(shape[0] % sf), :, :]
            if shape[0] % sf > 0
            else self.input_image
        )
        self.input_image = (
            self.input_image[:, : -(shape[1] % sf), :]
            if shape[1] % sf > 0
            else self.input_image
        )
    ...
    def get_top_left(self, size, for_g, idx):
        ...
        top, left = (
            min(max(0, row - size // 2), self.in_rows - size),
            min(max(0, col - size // 2), self.in_cols - size),
        )
